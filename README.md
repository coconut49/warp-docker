...
- `BETA_FIX_HOST_CONNECTIVITY`: If set, the entrypoint will apply a fix at startup and healthchecks will keep it updated to preserve host connectivity. The fix ensures traffic entering through non-WARP interfaces leaves via the same interface. See [host connectivity issue](docs/host-connectivity.md) for more information.
